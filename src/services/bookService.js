...
function query(filterBy = null) {
    ...
    if (filterBy) {
        var { title } = filterBy
        booksToReturn = gBooks.filter(book => book.title.toLowerCase().includes(title.toLowerCase())
        )
    }
...
function save(bookToSave) {
    if (bookToSave.id) {
        const idx = gBooks.findIndex(book => book.id === bookToSave.id)
        gBooks.splice(idx, 1, bookToSave)

    } else {
        bookToSave.id = _makeId()
        // userToSave.imgName = '1'
